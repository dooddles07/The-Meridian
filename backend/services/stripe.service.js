...
// items when there's a non-refundable fee on top of the refundable deposit
// (e.g. Verandah: $200 fee + $400 deposit = $600; Move-In/Out: $200 + $2000),
// so the resident sees exactly what's refundable right on Stripe's own page.
// kind ('booking' | 'move') + id travel in metadata (not the URL) so the
// webhook can look up the right model without trusting anything client-sent.
async function createDepositCheckoutSession({ kind, id, label, amount, bookingFee = 0, refundableAmount, residentEmail, depositDueAt }) {
  ...
  const lineItems = bookingFee > 0
    ? [
        { quantity: 1, price_data: { currency: 'usd', unit_amount: toCents(bookingFee), product_data: { name: `${label} — Booking Fee (non-refundable)` } } },
        { quantity: 1, price_data: { currency: 'usd', unit_amount: toCents(refundable), product_data: { name: `${label} — Refundable Deposit` } } },
      ]
    : [
        { quantity: 1, price_data: { currency: 'usd', unit_amount: toCents(amount), product_data: { name: `${label} — Refundable Deposit` } } },
      ];
  return stripe.checkout.sessions.create({
    ...
    success_url: `${APP_URL}/portal.html?paid=1&kind=${encodeURIComponent(kind)}&id=${encodeURIComponent(id)}`,
